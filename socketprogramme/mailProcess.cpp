...
std::string getAccessToken(const std::string& code, const std::string& clientId, const std::string& clientSecret, std::string& refreshtoken) {
    ...
    if (curl) {
        ...
        // Bật verbose để theo dõi quá trình gửi yêu cầu
        ...
        if (res != CURLE_OK) {
            ...
            // Phân tích phản hồi JSON để lấy access_token
...
std::string readEmailContent(const std::string& accessToken, const std::string& messageId) {
    CURL* curl;
    CURLcode res;
    curl = curl_easy_init();
    std::string responseString;
    ...
    if (curl) {
        ...
        if (res != CURLE_OK) {
            ...
            //std::cout << "Gmail Email Content Response: " << responseString << std::endl;
...
std::string readGmail(const std::string& accessToken, int& num) {
    CURL* curl;
    CURLcode res;
    curl = curl_easy_init();
    std::string newest;
            ...
            try {
                ...
                if (jsonResponse.contains("messages")) {
                    ...
                    for (const auto& message : messages) {
                        ...
                        newest = readEmailContent(accessToken, message["id"]);
